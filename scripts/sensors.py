...
try:
    ...
    try:
        ...
        # Display sensor data
        y_pos = 10
        draw.text((10, y_pos), 'Sensor Data', font=font24, fill=epd.BLACK)
        y_pos += 35

        # Display each sensor reading
        for key, value in sensor_data.items():
            display_text = f"{key}: {value}"
            draw.text((10, y_pos), display_text, font=font15, fill=epd.BLACK)
            y_pos += 20
        ...
        Himage = Image.new('RGB', (epd.height, epd.width), epd.WHITE)
        draw = ImageDraw.Draw(Himage)
        draw.text((10, 10), 'Error fetching sensor data', font=font18, fill=epd.RED)
        draw.text((10, 40), str(e), font=font15, fill=epd.BLACK)

    epd.display(epd.getbuffer(Himage))
    time.sleep(10)  # Show sensor data longer
